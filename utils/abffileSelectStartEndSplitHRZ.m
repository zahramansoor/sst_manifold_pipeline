...
if ~new
    ...
    subplot(2,1,2);hold on;plot(data(:,5));plot(abs(diff(data(:,5)))>0.3*max(abs(diff(data(:,5)))),'r');
    ...
    forwardvel=-0.013*data(scanstart:scanstop,4);   %120806 EH
...
for n=1:numfiles
    ...
    save(fullFfile,'ybinned','numframes','rewards','galvo','forwardvel','-append'); %131018 added galvobinned
    %     if size(data,2)>7%131018
